"""Unit test for the QA LLM-path indirect-prompt-injection defense (P0#4
cheap part, mục 9). No real network call happens -- the LLM is monkeypatched,
matching the pattern already used for plan_builder/reflection_engine's LLM
paths (test_plan_builder_llm.py, test_reflection_engine_llm.py)."""
...
def test_qa_v1_prompt_declares_context_chunks_inert():
    """The real (unmocked) prompt file must actually carry the defense rule
    -- this is the shipped deliverable, not a test double."""
    real_prompt = qa_answer_service.PROMPT_PATH.read_text(encoding="utf-8")
    assert "<context_chunk>" in real_prompt
    assert "instruction" in real_prompt.lower()
    assert "not" in real_prompt.lower() or "never" in real_prompt.lower()


def test_llm_call_wraps_chunk_text_in_context_chunk_tags_and_warns_it_is_untrusted(monkeypatch):
    ...
    delimited and flagged as untrusted data in the actual message sent to
    the LLM -- not left indistinguishable from the surrounding instructions
    -- so qa_v1.md rule 8 has something concrete to point at."""
    injected = (
    ...
    def _fake_invoke(messages):
        ...
        return LlmQaPayload(answer="ok", cited_chunk_ids=["INJ-1"], insufficient_context=False)
    ...
    service._answer_with_llm(question="Ban co the giup gi?", subject_code="SSA101", retrieved=retrieved)

    messages = captured["messages"]
    user_message = next(m["content"] for m in messages if m["role"] == "user")

    assert '<context_chunk id="INJ-1">' in user_message
    assert "</context_chunk>" in user_message
    ...
    start = user_message.index('<context_chunk id="INJ-1">')
    end = user_message.index("</context_chunk>", start)

...
def load_dotenv_fallback():
    """Manual fallback to load .env when python-dotenv is not installed."""
    env_path = Path(".env")
...
def _restore_orphaned_pending() -> None:
    """Find any session.pending.*.jsonl files in LOG_DIR and restore them to LOG_FILE."""
    if not LOG_DIR.exists():
        return
    for p in LOG_DIR.glob("session.pending.*.jsonl"):
        print(f"[ai-log] Found orphaned pending file: {p.name}, restoring it...", file=sys.stderr)
        _restore_pending(p)
...
def main():
    ...
    # First, heal any orphaned pending files from previous failed/interrupted runs
    _restore_orphaned_pending()

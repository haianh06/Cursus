...
def build_entry(msg: dict, repo: str, branch: str, commit: str,
    ...
    event = msg.get("event", "UserPrompt")
    sub_index = msg.get("sub_index", 0)
    entry_id = f"antigravity-{msg['conv_id']}-{msg['step_index']:05d}"
    if event == "PostToolUse":
        entry_id += f"-{sub_index}"

    prompt = msg["text"] if event == "UserPrompt" else ""
    if event == "PostToolUse" and isinstance(msg.get("tool_input"), dict):
        prompt = msg["tool_input"].get("Prompt") or msg["tool_input"].get("prompt") or ""
    ...
    return {
        ...
        "student": student,
        "prompt": prompt,
        "tool_name": msg["text"] if event == "PostToolUse" else "",
        "tool_input": msg.get("tool_input") if event == "PostToolUse" else None,
        "response_summary": "",
    }

> 📌 **Bản gốc nguyên văn của BTC — giữ lại để đối chiếu, không dùng làm README thật.** Đây là nội dung `README.md` y hệt lúc BTC cấp template (initial commit `4741ab8`), trước khi team viết đè thành README thật của Cursus. README đang hoạt động nằm ở [`/README.md`](../../../README.md). Xem `docs/project/structure-btc.md` để biết toàn bộ file BTC cấp sẵn khác.
